...
{
public:
	Metal(const glm::vec3& albedo, float fuzz) : albedo{ std::make_shared<ColorSampler>(albedo) } {}
...
{
public:
	Dielectric(const glm::vec3& albedo, float refractionIndex) : albedo{ std::make_shared<ColorSampler>(albedo) } {}
	Dielectric(std::shared_ptr<Sampler> albedo) : albedo{ albedo }, refractionIndex{ refractionIndex } {}
	virtual bool Scatter(const ray_t& ray, const raycastHit_t& hit, glm::vec3& attenuation, ray_t& scattered) const override;
